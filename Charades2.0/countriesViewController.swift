...
class countriesViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    var shuffledArrayCountried = [String]()
    var color = [UIColor]()
    var score = 0
    var buttonTag = 0
    ...
    let countries =  ["Afghanistan", "Albania", "Algeria", "Andorra", "Angola", "Argentina", "Armenia", "Australia", "Austria", "Bahamas", "Bangladesh", "Barbados", "Belgium", "Belize", "Bolivia", "Bosnia", "Herzegovina", "Botswana", "Brazil", "Bulgaria", "Cambodia", "Canada", "Chad", "Chile", "China", "Colombia", "Costa Rica", "Croatia", "Cuba", "Czech Republic", "Denmark", "Domincan Republic", "Egypt", "Fiji", "Finland", "France", "Germany", "Greece", "Guatemala", "Haiti", "Hungary", "Iceland", "India", "Indonesia", "Iran", "Iraq", "Ireland", "Israel", "Italy", "Jamaica", "Japan", "Kenya", "Libya", "Liechtenstein", "Luxembourg", "Madagascar", "Mexico", "Mongolia", "Montenegro", "Netherlands", "New Zealand", "North Korea", "Norway", "Pakistan", "Panama", "Peru", "Philippines", "Poland", "Portugal", "Russia", "Rwanda", "Serbia", "Sierra Leon", "Singapore", "Slovakia", "Slovenia", "South Africa", "South Korea", "Swaziland", "Sweden", "Switzerland", "Syria", "Tanzania", "Thailand", "Turkey", "Uganda", "Ukraine", "United Kingdom", "United Arab Emirates", "USA", "Venezuela", "Vietnam", "Yemen", "Zambia", "Zimbabwe"]
    ...
    func newWord(shuffling: [String]) {
        if countries.count > countTimesTapped {
            gameLabel.text = shuffling[countTimesTapped]
        } else {
            giveAnswer()
            print(shuffledArrayCountried)
        }
    }
    ...
    func shuffleArray() -> [String] {
        ...
        var upperLimit = countriesArray.count
        
        for _ in 1...countriesArray.count {
            randomNumber = Int(arc4random_uniform(UInt32(upperLimit)))
            shuffledArrayCountried.append(countriesArray[randomNumber])
            countriesArray.remove(at: randomNumber)
            upperLimit -= 1
        }
        return shuffledArrayCountried
    }
    ...
    @IBAction func startGameButton(_ sender: Any) {
        ...
        myTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { (timer) in
            ...
            if self.counter == 0 {
                ...
                self.giveAnswer()
                print(self.shuffledArrayCountried)
                self.player.pause()
            }
        }
        do
